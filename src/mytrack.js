...
L.Control.Watermark = L.Control.extend({ //download-button
  ...
  _download: function (ev) {
    var tmp = this._map._layers  //mytrack.gpx //must be named mytrack!
...
L.Control.Watermark2 = L.Control.extend({ //upload-button
  ...
  ajax: function (url) {
    ...
    fetch(url).then(function (response) {
      response.text().then(function (data) { thisLoader._add(data, response.headers.get("Last-modified")) }
      )
    })
  },
  _add: function (data, lm) {
   ...
   // this._map.panTo(mt.getBounds().getCenter())  //.fitBounds(mt.getBounds())
    ...
    mt = mt.bindPopup(function (layer) {
      var fe = layer.feature.geometry.coordinates, tot = 0
